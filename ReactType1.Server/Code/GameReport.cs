...
namespace ReactType1.Server.Code
{
    public class GameReport
    {
        ...
        public async Task<IDocument> CreateDocument(int weekid, DbLeagueApp db, string site)
        {
            ...
            List<MatchScheduleView> matches = [.. db.MatchScheduleViews.FromSql($"EXEC GameSchedule {weekid}")];
            

            List<AllTeamsView> teams = [.. db.AllTeamsViews.FromSql($"EXEC AllTeams {league.Id}")];
                ...
                .Page(page =>
                {
                    ...
                    page.Header()
                           ...
                           .Column(column =>
                           {
                               ...
                               column.Item().Text($"Rinks for {schedule?.GameDate.ToShortDateString()} Playoff Games");
                    ...
                    page.Content().PaddingVertical(20).Column(column =>
                    {
                        ...
                        column.Item().AlignCenter().Table(table =>
                        {
                            ...
                            var thisweek = matches.Where(x => x.GameDate == schedule?.GameDate).ToList();
                            table.Cell().Element(CellStyle).Text(schedule.GameDate.ToShortDateString()).FontSize(8);
